...
describe("shorthand syntax - string attributes", async () => {
  beforeAll(() =>
    context.subject(
  ...
  test("applies the default value from controller", async () => {
    const { getTestElement, subjectController } = await context.testDOM(`
      <div data-controller="subject">
        <div sx-data-output="subject#stringValue" data-test-element="target"></div>
      </div>
    `);
  ...
  test("overrides the existing attribute value", async () => {
    const { getTestElement, subjectController } = await context.testDOM(`
      <div data-controller="subject">
        <div data-output="foo" sx-data-output="subject#stringValue" data-test-element="target"></div>
      </div>
    `);
  ...
  test("doesn't remove attributes when the value is an empty string", async () => {
    const { getTestElement } = await context.testDOM(`
      <div data-controller="subject">
        <div sx-data-output="subject#emptyStringValue" data-test-element="target"></div>
      </div>
    `);
...
describe("shorthand syntax - boolean attributes", async () => {
  beforeAll(() =>
    context.subject(
      class extends Controller {
        static values = {
  ...
  test("adds or removes the attribute from the element", async () => {
    const { getTestElement, subjectController } = await context.testDOM(`
      <details data-controller="subject">
        <summary open sx-open="subject#booleanValue" data-test-element="summary">Summary</summary>
        <div>Details</div>
      </details>
    `);
...
describe("shorthand syntax - classes", async () => {
  beforeAll(() =>
    context.subject(
      class extends Controller {
        static values = {
  ...
  test("can resolve class objects", async () => {
    const { getTestElement, subjectController } = await context.testDOM(`
      <div data-controller="subject">
        <span sx-class="subject#themeStylesAsObject" data-test-element="target"></span>
      </div>
    `);
  ...
  test("can resolve class arrays", async () => {
    const { getTestElement, subjectController } = await context.testDOM(`
      <div data-controller="subject">
        <span sx-class="subject#themeStylesAsArray" data-test-element="target"></span>
      </div>
    `);
    const target = getTestElement("target");
  ...
  test("can resolve class strings", async () => {
    const { getTestElement, subjectController } = await context.testDOM(`
      <div data-controller="subject">
        <span sx-class="subject#themeStylesAsString" data-test-element="target"></span>
      </div>
    `);
    const target = getTestElement("target");
  ...
  test("doesn't overwrite existing classes", async () => {
    const { getTestElement, subjectController } = await context.testDOM(`
      <div data-controller="subject">
        <span class="border-hotpink" sx-class="subject#themeStylesAsString" data-test-element="target"></span>
      </div>
    `);
    const target = getTestElement("target");
